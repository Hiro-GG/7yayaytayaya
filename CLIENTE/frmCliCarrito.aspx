...
<asp:Content ID="Content1" ContentPlaceHolderID="headContent" runat="server">
    <style>
        ...
        .carrito-header {
            ...
            padding: 35px 20px 45px; /* Agregado padding lateral de 20px */
            margin-bottom: 20px;
            margin-top: 20px;       /* Un poco de espacio arriba */
            border-radius: 32px;    /* Redondeado completo */
        ...
        .carrito-header::after {
            ...
            left: 10%;
            right: 10%;
            bottom: -8px;     /* también más pequeño */
            height: 16px;
        ...
        .btn-nueva-compra {
            ...
            background-image: linear-gradient(135deg, #ff9800, #ff7043);
            color: #fff !important;
        ...
        /* 🔥 Asegurar que los botones de servicio SIEMPRE se vean */
        .servicio-footer .btn-detalle,
        .servicio-footer .btn-agregar {
            opacity: 1 !important;
    ...
    <script type="text/javascript">
        function validarFechasViaje() {
            ...
            // Fechas de vuelo
            var pnlVuelo = document.getElementById('<%= pnlFechasVuelo.ClientID %>');
            if (pnlVuelo && pnlVuelo.style.display !== 'none') {
                var txtVueloIda = document.getElementById('<%= txtVueloIda.ClientID %>');
                if (txtVueloIda && !txtVueloIda.value) {
                    hayError = true;
                    mensaje += '- Selecciona la fecha de salida del vuelo.\n';
                }
            }

            // Fechas de hotel
            var pnlHotel = document.getElementById('<%= pnlFechasHotel.ClientID %>');
            if (pnlHotel && pnlHotel.style.display !== 'none') {
                var txtHotelCheckIn = document.getElementById('<%= txtHotelCheckIn.ClientID %>');
                var txtHotelCheckOut = document.getElementById('<%= txtHotelCheckOut.ClientID %>');
                if (!txtHotelCheckIn.value || !txtHotelCheckOut.value) {
                    hayError = true;
                    mensaje += '- Selecciona las fechas de check-in y check-out del hotel.\n';
                }
            }

            // Fecha de tour
            var pnlTour = document.getElementById('<%= pnlFechasTour.ClientID %>');
            if (pnlTour && pnlTour.style.display !== 'none') {
                var txtFechaTour = document.getElementById('<%= txtFechaTour.ClientID %>');
                if (!txtFechaTour.value) {
                    hayError = true;
                    mensaje += '- Selecciona la fecha del tour o actividad.\n';
                }
            }

            if (hayError) {
                alert('Antes de confirmar la compra, completa las fechas:\n\n' + mensaje);
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="container pb-4">
        ...
        <asp:Panel ID="pnlSinCompra" runat="server" CssClass="pnl-sin-compra" Visible="false">
            ...
            y luego selecciona los servicios que desees (vuelos, hoteles y tours).<br /><br />

            <asp:Button ID="btnIrMisReservas" runat="server"
                Text="Ir a ver mis reservas"
                CssClass="btn btn-primary btn-sm"
                OnClick="btnIrMisReservas_Click" />
        </asp:Panel>

        <!-- Panel CON compra activa -->
                    ...
                    <div class="order-summary-card">
                        ...
                        <div class="order-actions">
                            <asp:Button ID="btnConfirmarCompra" runat="server"
                                ...
                                OnClientClick="return validarFechasViaje();" />

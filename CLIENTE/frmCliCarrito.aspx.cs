    ...
    public partial class Formulario_web12 : System.Web.UI.Page
    {
        ...
        protected void rptServicios_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            if (e.CommandName == "VerDetalle")
            {
                ...
                string script = @"
                    window.open('frmCliDetServicio.aspx', 'detalleServicio',
                    'width=360,height=640,resizable=yes,scrollbars=yes');";

                ClientScript.RegisterStartupScript(
                    this.GetType(),
                    "PopupDetalleServicio",
                    script,
                    true
                );
        ...
        #region Validación de fechas (server side)

        private bool ValidarFechasRequeridas()
        {
            bool ok = true;

            // Si hay vuelos, fecha de ida obligatoria
            if (pnlFechasVuelo.Visible)
            {
                if (string.IsNullOrWhiteSpace(txtVueloIda.Text))
                    ok = false;
            }

            // Si hay hotel, check-in y check-out obligatorios
            if (pnlFechasHotel.Visible)
            {
                if (string.IsNullOrWhiteSpace(txtHotelCheckIn.Text) ||
                    string.IsNullOrWhiteSpace(txtHotelCheckOut.Text))
                    ok = false;
            }

            // Si hay tour, su fecha obligatoria
            if (pnlFechasTour.Visible)
            {
                if (string.IsNullOrWhiteSpace(txtFechaTour.Text))
                    ok = false;
            }
        ...
        protected void btnConfirmarCompra_Click(object sender, EventArgs e)
        {
            ...
            // Validar fechas antes de confirmar (además del JS por si desactivan scripts)
            if (!ValidarFechasRequeridas())
            {
                ClientScript.RegisterStartupScript(
                    this.GetType(),
                    "FechasIncompletas",
                    "alert('Para confirmar la compra debes capturar las fechas de los servicios seleccionados.');",
                    true);
                return;
            }
            ...
            ClientScript.RegisterStartupScript(
                this.GetType(),
                "CompraConfirmada",
                "alert('Compra confirmada correctamente. Puedes consultar tu reserva en el apartado Mis Reservas.');",
                true);
        }
        ...
        #region Ir a Mis Reservas

        protected void btnIrMisReservas_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/CLIENTE/frmCliMisCompras.aspx");
        }
